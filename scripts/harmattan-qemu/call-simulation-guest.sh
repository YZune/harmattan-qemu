...
test "\$value" = blanco
$audio_env $helpers/n00-call-simulation ${N00_CALL_AUDIO:-off} >$root/backend.log 2>&1 &
...
DBUS_SYSTEM_BUS_ADDRESS=$address QT_GRAPHICSSYSTEM=raster M_FORCE_LOCAL_THEME=1 \\
...
EOF
        ...
        sh "$0" report
        ;;
    incoming|end|report|invalid-input|busy|stale)
        if [ "$(id -u)" = 0 ]; then exec su user -c "sh $0 $1"; fi
        test "$(id -u)" = 29999
        ...
        ;;
    stop)
        if [ "$(id -u)" = 0 ]; then exec su user -c "sh $0 stop"; fi
        test "$(id -u)" = 29999
        backend=$(owned_pid backend "$helpers/n00-call-simulation")
        ui=$(owned_pid ui /usr/bin/call-ui)
        gconf=$(owned_pid gconf /usr/lib/gconf2/gconfd-2)
