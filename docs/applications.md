...
An explicit user profile also enables a narrowly scoped adapter for the recorded **FBReader 0.99.5** executable. Before enabling it, Home checks the executable, stock Qt 4.7.4 libraries and helper hashes. In that executable only, the adapter asks Qt to supply a plain `QWidget` viewport and dispose of the unused `QGLWidget`; the original application still owns its text layout, pages and controls. Other processes and ordinary widgets pass through unchanged. Unknown FBReader versions are skipped, while an unexpected Qt/helper identity fails startup. The source builder and future prebuilt-helper manifest include this adapter; an already downloaded preview app is not updated automatically.

## Local function validation

The [installation record](applications-validation.json) covers package configuration and the original desktop. The later [daily application record](daily-applications-validation.json) and [reader record](reader-validation.json) cover these function checks:
...
| FBReader 0.99.5 | PASS for the recorded local workflow | Discover a local EPUB in Books; open it through the original library; tap forward/back; restore the same page after application and guest restarts |

These checks used QMP pointer events in a headless guest, original application identities, captured UI states and saved-file hashes. A second boot verified the same profile and clean GPU teardown. The earlier ownNotes/Filebox change passed 285 host tests and the audio-enabled Home/Notes/Maliit/Calculator/transition regression. Physical mouse input in a Cocoa window remains untested for these third-party functions because the Mac was locked. The private bounded exploration driver is not a shipped automated application diagnostic.

The later [reader validation](reader-validation.json) covers the pinned viewport adapter, 290 host tests, freshly built and packaged helpers, and all three applications in the same profile. Reader page crops match after forward/back navigation, reopening and a guest restart. Its original database records the tested paragraph position. The existing three-client Home GPU gate remains intact; a separate check requires each reader's additional context to terminate and disconnect completely before reuse, and still rejects unknown calls, warnings or incomplete shutdown.
...
For the validated reader workflow, place an EPUB in the guest's `/home/user/MyDocs/Books` before launching FBReader. In the library, choose **By Author**, the author, the book, then the menu's **Read book** action. Tap the right/left side to turn pages. Horizontal swipe navigation and the **Add Book** file picker's directory behavior did not pass this bounded check; do not infer those functions from the reading result. DRM, network libraries and arbitrary document formats remain outside the accepted scope.
