...
本次普通来电与锁屏来电回归在关闭音频的条件下通过状态、像素和严格 GPU 检查。早期铃声检查单独保留；本次图形修复不代表重新验收音频或物理输入。见[当前组合验证](call-lockscreen-validation.json)。
...
诊断检查原版 UI、辅助程序和配置服务的身份与进程连续性、状态转换、旧通道与非法请求拒绝、原版按钮像素、缺失资源标记，以及独立的四上下文 GLES 生命周期。既有桌面 GPU 验证器保持不变；故障、非法调用、警告、缺失上下文和错误退出顺序仍会导致失败。启用音频时从私有输出监视器记录约 32 秒声音，同时检查全段和尾部，属于软件输出证据，不等同于声学测量或物理输入验收。实际执行的检查见[早期音频验证](call-simulation-validation.json)和[当前图形回归](call-lockscreen-validation.json)。
...
限域像素图适配还将 `qDrawBorderPixmap` 的各条源边限制在源图范围内：原版 slidehint 图片高 8 像素，CSS 却指定 10 像素上边框。目标边框仍为 10 像素，由原 Qt 缩放有效源像素；原图片文件、CSS、布局与动画参数不变。合成器另在直接绘制切换期间保留软件回读纹理，直到取得有效替代帧；真正的 EGL 图像释放与纹理池回收仍走原路径。

组合诊断区分锁屏、来电状态、按钮像素和严格图形门禁，并保留连续动画帧供检查。新增横幅及六张运动帧的全部十行上边框检查，拒绝非绿色或横向不一致的像素。它不测量动画帧率，不替代可见 Cocoa 操作、真实声音或物理电话验收。

本次锁屏组合、普通来电与锁屏回归的结果见[锁屏来电验证记录](call-lockscreen-validation.json)。这些无窗口检查中，空纹理警告和彩色横幅边缘行均已解决；严格 GPU 验证器保持不变。
