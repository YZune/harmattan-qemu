...
这是救援桌面中的锁屏界面接入，未恢复 MCE、PIN／设备安全锁、闲置自动锁定、物理电源键 GPIO 事件、屏幕省电或硬件休眠恢复。待机时钟使用原版低功耗**界面模式**，QEMU 仍在运行；未启用原版完全关闭显示模式。独立的[锁屏来电实验](call-simulation.zh-CN.md#锁屏来电)将原来电事件接到此界面。通用通知唤醒仍不在验收范围内。
