...
The [package installer](applications.md) transfers reviewed ARMEL packages into an explicit profile, preserves dependency failures and restores SDK graphics links after package triggers. The [installation record](applications-validation.json) covers dependency configuration and restart. The later [daily application record](daily-applications-validation.json) covers 285 host tests, Home-launched ownNotes editing and Filebox copying, exact saved data after a profile restart, and the audio-enabled UI regression. These function checks used headless QMP input; physical Cocoa input remains untested. FBReader still fails through its explicit OpenGL viewport.
