...
- 原始下载物和准备好的客体基础盘，与一次性运行目录分开保留。[客体准备](guest-inputs.zh-CN.md)脚本虽然公开，仍需要准确的外部原始材料；GitHub 不是完整运行环境的备份。
- 原生启动器默认使用每次运行独立的克隆和快照，Notes 等会话内修改在退出后丢弃；显式选择[用户档案](storage.zh-CN.md)可保留已保存文件。诊断使用独立磁盘。历史启动脚本可能不同，应使用构建说明中的原生入口。
